...
### 3. Strict Target-Driven Investigation & Mandatory Researcher Subagent (MANDATORY)
To minimize token consumption, avoid useless web search loops, and prevent cluttering the main conversation:
- **Mandatory `researcher` Subagent Delegation**: Whenever research is required — whether gathering context from the local repository, checking official documentation, or looking up external APIs/web references — you **MUST** invoke the specialized `researcher` subagent (`invoke_subagent` with `TypeName: "researcher"`). Do not perform multi-step search/read loops directly in the main conversation context.
- **No Blind Scanning**: Do not list directories recursively, search blindly, or read files unless they are directly and explicitly related to the task. If you are asked to modify a specific file, read *only* that file.
...
- **Thought-First Investigation Planning**: Before delegating or using any investigation tool, write down a quick target plan in your thinking block: *What specific information/file do I need to find, and why is it necessary for this task?*
- **Strict Web Search Constraints**: Web searches (delegated to `researcher`) are strictly for technical documentation, syntax reference, or specific compiler/interpreter error messages. Do NOT perform general-knowledge or non-technical searches.
- **Strict Workspace Boundaries**: Never list, search, or read files outside the user's workspace (e.g., user home folder, system directories, App Data).
